...
/*   Updated: 2022/01/15 15:36:51 by trurgot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	my_make_step_0(t_solong *game, int x, int y)
{
	...
	if (x == 1)
		mlx_put_image_to_window(game->mlx, game->mlx_win, game->playerr.img, \
		(game->pl_posx + x) * game->bits, (game->pl_posy + y) * game->bits);
	else if (x == -1)
		mlx_put_image_to_window(game->mlx, game->mlx_win, game->playerl.img, \
		(game->pl_posx + x) * game->bits, (game->pl_posy + y) * game->bits);
	else if (y == -1)
		mlx_put_image_to_window(game->mlx, game->mlx_win, game->playerb.img, \
		(game->pl_posx + x) * game->bits, (game->pl_posy + y) * game->bits);
	else
...
static int	my_make_step_e(t_solong *game, int x, int y)
{
	if (game->qty_collect_rest == 0)
	{
		mlx_put_image_to_window(game->mlx, game->mlx_win, game->exit_open.img, \
		(game->pl_posx + x) * game->bits, (game->pl_posy + y) * game->bits);
		mlx_put_image_to_window(game->mlx, game->mlx_win, game->empty.img, \
		(game->pl_posx) * game->bits, (game->pl_posy) * game->bits);
		game->game_over = 1;
...
static int	my_make_step(t_solong *game, int x, int y)
{
	if (game->map_components[game->pl_posy + y][game->pl_posx + x] == '1')
		...
		return (my_make_step_e(game, x, y));
	else if (game->map_components[game->pl_posy + y][game->pl_posx + x] == 'A')
	{
		mlx_put_image_to_window(game->mlx, game->mlx_win, game->explosn.img, \
		(game->pl_posx) * game->bits, (game->pl_posy) * game->bits);
		game->game_over = 1;
		return (0);
	}
