...
/*   Updated: 2022/01/15 16:50:11 by trurgot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	key_hook(int keycode, t_solong *game)
{
	...
	(game->game_over == 0))
	{
		...
		{
			ft_putnbr(++game->qty_steps);
			write(1, "\n", 1);
		}
		if ((game->game_over != 0))
		{
			ft_putstr_fd("The game over\nQuantity of steps is: ", 1);
			ft_putnbr(game->qty_steps);
			ft_putstr_fd("\nFound ", 1);
			ft_putnbr(game->qty_collect);
			ft_putstr_fd(" collectibles\nPress Esc for quit\n", 1);
		}
	}
	if (keycode == 53)
	{
